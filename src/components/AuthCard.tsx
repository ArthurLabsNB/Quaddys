...
export default function AuthCard() {
  ...
  async function oauth(provider: "google" | "github" | "discord") {
    ...
    try {
      ...
      const errMsg = e instanceof Error ? e.message : "No se pudo iniciar sesión con OAuth";
      setErr(errMsg);
    } finally {
      setPending(false);
    }
  ...
  return (
    <motion.div
      ...
      className="relative w-[min(100%,440px)] rounded-2xl p-[2px] shadow-[0_10px_40px_-15px_rgba(0,0,0,0.7)]"
      aria-live="polite"
    >
      ...
      <div
        ...
        style={{
          background:
            "conic-gradient(from 0deg, rgba(99,102,241,.35), rgba(56,189,248,.35), rgba(168,85,247,.35), rgba(99,102,241,.35))",
          maskImage: "radial-gradient(closest-side, rgba(0,0,0,.8), rgba(0,0,0,0))",
        }}
      />
      <div className="rounded-2xl bg-white/10 backdrop-blur-xl border border-white/10 p-5">
        <header className="mb-4 text-center">
          <h1 className="text-2xl font-semibold text-white tracking-tight">Portal de Quaddys</h1>
          <p className="text-sm text-white/70">Tu mente es tu mejor carta.</p>
        </header>
                  ...
                  <div className="mt-1 h-1 w-full bg-white/10 rounded">
                    <div
                      className={`h-1 rounded ${
                        pass.length >= 10 ? "w-3/3" : pass.length >= 8 ? "w-2/3" : "w-1/3"
                      }`}
        ...
        <div className="grid grid-cols-3 gap-2">
          <button
            ...
            disabled={pending}
            className="rounded-lg bg-white/90 text-black py-2 text-sm font-medium hover:bg-white"
          >
            Google
          </button>
          <button
        ...
        {err && (
          <motion.div
            ...
            initial={{ opacity: 0, y: 6 }}
            animate={{ opacity: 1, y: 0 }}
            className="mt-4 rounded-lg bg-red-500/15 border border-red-400/20 p-3 text-red-200 text-sm"
          >
            {err}
          </motion.div>
        )}
        {msg && (
          <motion.div
            ...
            initial={{ opacity: 0, y: 6 }}
            animate={{ opacity: 1, y: 0 }}
            className="mt-4 rounded-lg bg-emerald-500/15 border border-emerald-400/20 p-3 text-emerald-200 text-sm"
          >
            {msg}
          </motion.div>
        )}
        ...
        <footer className="mt-4 flex items-center justify-between text-xs text-white/60">
          ...
          <a className="hover:text-white" href="mailto:soporte@quaddys.app?subject=Ayuda%20de%20acceso">
            ¿Problemas? Soporte
          </a>
        </footer>
      </div>
